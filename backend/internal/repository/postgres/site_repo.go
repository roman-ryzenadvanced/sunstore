...
// CreateSiteOrder inserts a site-scoped order plus its line items in one transaction.
func (d *DB) CreateSiteOrder(ctx context.Context, input domain.CreateSiteOrderInput) (*domain.SiteOrder, error) {
	var order domain.SiteOrder

	err := d.WithTx(ctx, func(tx pgx.Tx) error {
		const insertOrder = `
			INSERT INTO site_orders (
				site_id, tbank_order_id, customer_name, customer_email, customer_phone, total_amount_kopecks, status
			) VALUES ($1,$2,$3,$4,$5,$6,$7)
			RETURNING id, site_id, tbank_order_id, tbank_payment_id, customer_name, customer_email, customer_phone,
			          total_amount_kopecks, status, raw_tbank_response, created_at, updated_at
		`
		if err := tx.QueryRow(ctx, insertOrder,
			...
			return mapWriteError("create site order", err)
		}

		for _, item := range input.Items {
			if _, err := tx.Exec(ctx, `
				INSERT INTO site_order_items (order_id, product_id, quantity, price_at_purchase_kopecks)
				VALUES ($1,$2,$3,$4)
			`, order.ID, item.ProductID, item.Quantity, item.PriceAtPurchaseKopecks); err != nil {
				return fmt.Errorf("create site order item: %w", err)
			}
		}
		return nil
	})
...
// AttachSitePaymentInit stores the result of a T-Bank init call on a site order.
func (d *DB) AttachSitePaymentInit(ctx context.Context, orderID int64, paymentID string, status domain.OrderStatus, raw json.RawMessage) error {
	tag, err := d.Pool.Exec(ctx, `
		UPDATE site_orders
	...
	if err != nil {
		return fmt.Errorf("attach site payment init: %w", err)
	}
	if tag.RowsAffected() == 0 {
...
// UpdateSiteOrderStatusByTBankOrderID updates a site order by the external T-Bank order id.
func (d *DB) UpdateSiteOrderStatusByTBankOrderID(ctx context.Context, tbankOrderID string, paymentID *string, status domain.OrderStatus, raw json.RawMessage) error {
	tag, err := d.Pool.Exec(ctx, `
		UPDATE site_orders
		SET
	...
	if err != nil {
		return fmt.Errorf("update site order status by tbank order id: %w", err)
	}
	if tag.RowsAffected() == 0 {
		return domain.ErrNotFound
	}
